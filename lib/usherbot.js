...
function getAllMovies(convo) {
	...
	doQuery(query).then(function(response) {
		...
		console.log('getAllMovies fucked up');
	});
};
...
function incrementVote(recordID) {
	...
	doQuery(query).then(function(response) {
		...
		console.log('incrementVote fucked up');
	});
};
...
function addTitle(titleName) {
	...
	var query = 'INSERT INTO movies (title, votes) VALUES ("' + titleName.toString() + '", ' + 1 + ');';
	console.log(query);
	doQuery(query).then(function(response) {
		console.log('title added');
	}, function(error) {
		console.log('addTitle fucked up', error);
	});
};
