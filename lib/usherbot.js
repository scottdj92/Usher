...
var StreamIt = require('canistreamit');
...
u.isVotingOpen = function () {
	...
	if(today.getDay() === 5) {
		//check to see if it is 9AM or later
...
// CHECK TO SEE IF MOVIE IS AVAILABLE TO STREAM
u.searchAvailableStream = function( movie ) {
    StreamIt.search(movie)
    .then(function(results) {
        //return StreamIt.streaming(results[0]);
        console.log(results);
    })
    .catch(function(error) {
        console.log('fetching stream failed', error);
    });
}
